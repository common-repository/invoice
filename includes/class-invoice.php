...
class Invoice {
    ...
    public function __construct() {
        ...
        add_filter("{$prefix}plugin_action_links_" . PI_PLUGIN_DIR_BASE, array($this, 'paypal_invoice_plugin_action_links'), 10, 4);
    }
    ...
    public function invoice_new_interval_cron_time($interval) {
     ...
     public function paypal_invoice_plugin_action_links($actions, $plugin_file, $plugin_data, $context) {
        $custom_actions = array(
            'configure' => sprintf('<a href="%s">%s</a>', admin_url('options-general.php?page=paypal-invoice-settings-option'), __('Configure', 'donation-button')),
            'docs' => sprintf('<a href="%s" target="_blank">%s</a>', 'https://www.premiumdev.com/product/paypal-invoice/', __('Docs', 'donation-button')),
            'support' => sprintf('<a href="%s" target="_blank">%s</a>', 'https://wordpress.org/support/plugin/invoice', __('Support', 'donation-button')),
            'review' => sprintf('<a href="%s" target="_blank">%s</a>', 'https://wordpress.org/support/view/plugin-reviews/invoice', __('Write a Review', 'donation-button')),
        );
